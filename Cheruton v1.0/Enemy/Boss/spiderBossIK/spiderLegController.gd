...
@onready var base_cast = $RayCast2D.target_position
@onready var base_offset_diag = $RayCast2D2.position.y
@onready var base_cast_diag = $RayCast2D2.target_position
...
func set_offset(vel):
	...
	ray_cast.target_position.x = base_cast.x + .2 * vel.x
	diag_ray_cast.position.y =  base_offset_diag + clamp(vel.y  * 1.3, -250, INF) # clamp so scaling up doesn't make spider reach very far
	diag_ray_cast.target_position = base_cast_diag + sign(vel.x)* (1 if sign(vel.x) == sign(base_cast_diag.x) else 0 ) *  Vector2(clamp(abs(vel.x)*.38 + int(not is_colliding_ground()) * 400, 0, INF), 0)  # if no where to place foot , boost diagonal vector
