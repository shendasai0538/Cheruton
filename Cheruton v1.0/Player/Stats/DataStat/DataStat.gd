...
func change_bar_colour(value, browse, fix):
	if(!browse):
		...
		if(value > bar.value || !DataResource.temp_dict_player[parent.active_tab.name + "_item"]):
			bar.set_tint_progress(Color(0, 1, 0))   # Green
			parent_rating_value.set("theme_override_colors/font_color",Color(0, 1, 0))
		elif(value < bar.value):
			bar.set_tint_progress(Color(1, 0, 0))   # Red
			parent_rating_value.set("theme_override_colors/font_color", Color(1, 0, 0))
