...
func _ready():
	...
	key_label.size = Vector2(8, 10) # default size for a single key
	base.size = Vector2(15, 15)
	if action_name:
		input_event = InputMap.action_get_events(action_name)[0]
	...
	if input_event is InputEventMouseButton:
		...
		key_label.size = string_size
		base.size = string_size + Vector2(9, 5)
		base.position = Vector2(-base.size.x * 2,-28) # centers to root take note of scaling of the sprite, offset affect by it.
